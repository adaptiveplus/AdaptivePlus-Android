package com.sprintsquads.adaptiveplusqaapp.utils

import android.content.Context
import com.google.gson.Gson
import com.sprintsquads.adaptiveplusqaapp.data.AdaptiveCustomIP
import com.sprintsquads.adaptiveplusqaapp.data.AdaptiveSdkEnvironment
import com.sprintsquads.adaptiveplusqaapp.data.Environment
import java.io.Serializable


private data class CustomIPsWrapper(
    val ips: List<AdaptiveCustomIP>
) : Serializable

fun addCustomIP(context: Context, ip: AdaptiveCustomIP) {
    val ips = getCustomIPs(context).toMutableList()
    ips.add(ip)

    updateCustomIPs(context, ips)
}

fun removeCustomIP(context: Context, ipName: String) {
    val ips = getCustomIPs(context).toMutableList()
    val remIndex = ips.indexOfFirst { "${it.name}: ${it.ip}" == ipName }
    if (remIndex != -1) {
        ips.removeAt(remIndex)
    }

    updateCustomIPs(context, ips)
}

fun getCustomIPBySpinnerName(context: Context, ipName: String): AdaptiveCustomIP? {
    val ips = getCustomIPs(context)

    return ips.firstOrNull { "${it.name}: ${it.ip}" == ipName }
}

private fun updateCustomIPs(context: Context, ips: List<AdaptiveCustomIP>) {
    val prefs = AppSharedPreferences(context)
    val jsonStr = Gson().toJson(CustomIPsWrapper(ips))

    prefs.saveString(AppSharedPreferences.ADAPTIVE_CUSTOM_IPS, jsonStr)
}

fun getCustomIPs(context: Context): List<AdaptiveCustomIP> {
    val prefs = AppSharedPreferences(context)
    val ipsWrapperStr = prefs.getString(AppSharedPreferences.ADAPTIVE_CUSTOM_IPS)

    val ips = try {
        Gson().fromJson(ipsWrapperStr, CustomIPsWrapper::class.java).ips
    } catch (e: Exception) {
        e.printStackTrace()
        listOf<AdaptiveCustomIP>()
    }.toMutableList()

    if (ips.firstOrNull { it.name == "NONE" } == null) {
        ips.add(
            AdaptiveCustomIP(name = "NONE", ip = "")
        )
    }

    return ips
}

private data class EnvsWrapper(
    val envs: List<AdaptiveSdkEnvironment>
) : Serializable

fun addNewTag(
    context: Context,
    envName: String,
    tagId: String,
    loadingType: AdaptiveSdkEnvironment.Tag.LoadingType,
    isInstructions: Boolean,
    isOnboarding: Boolean,
    hasBookmarks: Boolean
) {
    val envs = getEnvs(context).toMutableList()
    envs.firstOrNull { it.name == envName }?.run {
        tags = tags.toMutableList().apply {
            add(
                AdaptiveSdkEnvironment.Tag(
                    id = tagId,
                    loadingType = loadingType,
                    isInstructions = isInstructions,
                    isOnboarding = isOnboarding,
                    hasBookmarks = hasBookmarks
                )
            )
        }
    }

    updateEnvs(context, envs)
}

fun removeTag(context: Context, envName: String, tagId: String) {
    val envs = getEnvs(context).toMutableList()
    envs.firstOrNull { it.name == envName }?.run {
        tags = tags.toMutableList().apply {
            val index = indexOfFirst { it.id == tagId }
            if (index != -1) {
                removeAt(index)
            }
        }
    }

    updateEnvs(context, envs)
}

fun addNewEnv(context: Context, env: AdaptiveSdkEnvironment) {
    val envs = getEnvs(context).toMutableList()
    envs.add(env)

    updateEnvs(context, envs)
}

fun removeEnv(context: Context, envName: String) {
    val envs = getEnvs(context).toMutableList()
    val remIndex = envs.indexOfFirst { it.name == envName }
    if (remIndex != -1) {
        envs.removeAt(remIndex)
    }

    updateEnvs(context, envs)
}

private fun updateEnvs(context: Context, envs: List<AdaptiveSdkEnvironment>) {
    val prefs = AppSharedPreferences(context)
    val jsonStr = Gson().toJson(EnvsWrapper(envs))

    prefs.saveString(AppSharedPreferences.ADAPTIVE_ENVS, jsonStr)
}

fun getEnvs(context: Context): List<AdaptiveSdkEnvironment> {
    val prefs = AppSharedPreferences(context)
    val envsWrapperStr = prefs.getString(AppSharedPreferences.ADAPTIVE_ENVS)

    val envs = try {
        Gson().fromJson(envsWrapperStr, EnvsWrapper::class.java).envs
    } catch (e: Exception) {
        e.printStackTrace()
        listOf<AdaptiveSdkEnvironment>()
    }.toMutableList()

    for (localEnvName in Environment.values().map { it.value }) {
        if (envs.firstOrNull { it.name == localEnvName } == null) {
            getLocalEnvByName(localEnvName)?.let { env ->
                envs.add(env)
            }
        }
    }

    return envs
}

fun getEnvByName(context: Context, envName: String) : AdaptiveSdkEnvironment? {
    val envs = getEnvs(context)

    return envs.firstOrNull { it.name == envName }
}

private fun getLocalEnvByName(envName: String) : AdaptiveSdkEnvironment? {
    return when (envName) {
        Environment.STAGING.value -> AdaptiveSdkEnvironment(
            name = Environment.STAGING.value,
            appId = "com.s10s.adaptiveplussampleapp",
            companySecret = "",
            appSecret = "",
            baseApiUrl = "https://api-staging.adaptive.plus",
            tags = stagingTags
        )
        Environment.HOMEBANK_DEV.value -> AdaptiveSdkEnvironment(
            name = Environment.HOMEBANK_DEV.value,
            appId = "kz.kkb.homebank.dev",
            companySecret = "",
            appSecret = "",
            baseApiUrl = "https://test-adaptive-api.homebank.kz",
            tags = homebankTags
        )
        Environment.HOMEBANK_PROD.value -> AdaptiveSdkEnvironment(
            name = Environment.HOMEBANK_PROD.value,
            appId = "kz.kkb.homebank",
            companySecret = "",
            appSecret = "",
            baseApiUrl = "https://adaptive-api.homebank.kz",
            tags = homebankTags
        )
        Environment.MYCAR.value -> AdaptiveSdkEnvironment(
            name = Environment.MYCAR.value,
            appId = "kz.smartmk.mycars.android",
            companySecret = "",
            appSecret = "",
            baseApiUrl = "https://adaptive-api.mycar.kz",
            tags = mycarTags
        )
        Environment.MYCAR_PREPROD.value -> AdaptiveSdkEnvironment(
            name = Environment.MYCAR_PREPROD.value,
            appId = "ru.agima.mycars.android",
            companySecret = "",
            appSecret = "",
            baseApiUrl = "http://178.154.231.192:3030",
            tags = mycarPreprodTags
        )
        Environment.MOCK.value -> AdaptiveSdkEnvironment(
            name = Environment.MOCK.value,
            appId = "",
            companySecret = "",
            appSecret = "",
            baseApiUrl = "",
            tags = listOf()
        )
        else -> null
    }
}

private val stagingTags = listOf(
    AdaptiveSdkEnvironment.Tag(id = "5f85ffadb069f4001221c9f9", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f859fac6aa1a80012ee31fd", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f859fb96aa1a80012ee31fe", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f5f79d22e6c6500125c2e56", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f859f846aa1a80012ee31fb", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f859f946aa1a80012ee31fc", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f5f7a2b2e6c6500125c2e58", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f859f1f6aa1a80012ee31a9", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f859f386aa1a80012ee31aa", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f5f7a6d2e6c6500125c2e5a", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f5f7a9e2e6c6500125c2e5c", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f5f7a042e6c6500125c2e57", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f5f7a562e6c6500125c2e59", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f5f7a892e6c6500125c2e5b", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f894b3c44795c0012511f35", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5fc61e6fa2577e0018a8b4de", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY)
)

private val homebankTags = listOf(
    AdaptiveSdkEnvironment.Tag(id = "5f645dcb1fcfdf0018828dde", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f645d741fcfdf0018828ddd", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f645da672b21a0018d808b0", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f645df372b21a0018d808b1", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f645e0c1fcfdf0018828ddf", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f645e2e72b21a0018d808b2", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f645e471fcfdf0018828de0", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY)
)

private val mycarTags = listOf(
    AdaptiveSdkEnvironment.Tag(id = "5f871516851f910012576f44", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f871535851f910012576f45", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f871570851f910012576f46", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f8715b4851f910012576f48", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f87158a851f910012576f47", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY)
)

private val mycarPreprodTags = listOf(
    AdaptiveSdkEnvironment.Tag(id = "5f871570851f910012576f46", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5f871516851f910012576f44", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY),
    AdaptiveSdkEnvironment.Tag(id = "5fe3d8d6a49a340018f9932f", loadingType = AdaptiveSdkEnvironment.Tag.LoadingType.EMPTY, isInstructions = true)
)